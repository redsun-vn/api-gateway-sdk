...
import {
	LOYALTY_REWARD_DISCOUNT_APPLICABILITY_ENUM,
	LOYALTY_REWARD_DISCOUNT_MODE_ENUM,
	LOYALTY_REWARD_TYPE_ENUM,
} from '../../enum/loyalty-service';

export namespace ILoyaltyReward {
	export interface ILoyaltyRewardCreate {
	...
	export interface ILoyaltyRewardUpdate
		extends Omit<
			Partial<ILoyaltyRewardCreate>,
			'program_id' | 'rewardType' | 'rewardProducts'
		> {}

	export interface ILoyaltyRewardResponse extends BaseResponse {
		...
		rewardProducts?:
			| ILoyaltyRewardProduct.ILoyaltyRewardProductResponse[]
			| null;
	}
}
