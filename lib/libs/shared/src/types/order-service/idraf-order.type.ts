...
export namespace IDrafOrder {
	export interface ICreateDrafOrder {
	...
	export interface ICreateDraftCopy {
		staffId: string;
		draftId: string;
	}

	export interface ICreateDrafOrderKafka extends ICreateDrafOrder {
		shop_id: number & tags.Type<'uint64'>;
	}

	export interface IUpdateDrafOrder
		extends Omit<Partial<ICreateDrafOrder>, 'branch_id' | 'staff_id'> {}

	export interface IConfirmDrafOrder {
		warehouse_id: number & tags.Type<'uint64'>;
		price_book_id?: number & tags.Type<'uint64'>;
	}

	export interface IDetailDrafOrderResponse extends BaseResponse {
	...
	export interface IUpdateCart
		extends Omit<Partial<ICreateDrafOrder>, 'staff_id'> {
		cart_session_id: string;
	}
}
