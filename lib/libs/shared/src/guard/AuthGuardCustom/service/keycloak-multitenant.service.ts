...
import { KEYCLOAK_CONNECT_OPTIONS } from 'nest-keycloak-connect';
...
export class KeycloakMultiTenantService {
	...
	async get(
		...
		if (this.instances.has(realm)) {
			...
			return this.instances.get(realm) as any;
		} else {
	...
	async resolveAuthServerUrl(
		...
		if (!this.keycloakOpts.multiTenant.realmAuthServerUrlResolver) {
			return (this.keycloakOpts.authServerUrl ||
				...
				this.keycloakOpts['server-url']) as any;
		}

		// Resolve realm authServerUrl
		const resolvedAuthServerUrl =
			this.keycloakOpts.multiTenant.realmAuthServerUrlResolver(
				realm,
				request,
			) as any;
		const authServerUrl =
	...
	async resolveSecret(
		...
		const resolvedRealmSecret =
			this.keycloakOpts.multiTenant.realmSecretResolver(realm, request) as any;
